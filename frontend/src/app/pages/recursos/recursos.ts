...
const RECURSOS_API_BASE = `${API_BASE}/recursos`;
...
export class RecursosComponent {
  ...
  async cargarRecursos(): Promise<void> {
    ...
    try {
      const response = await fetch(`${RECURSOS_API_BASE}/leer`);
      if (!response.ok) {
  ...
  async eliminarRecurso(id: string | undefined): Promise<void> {
    ...
    try {
      const response = await fetch(`${RECURSOS_API_BASE}`, {
        method: 'DELETE',
        headers: this.authHeaders,
